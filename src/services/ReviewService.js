...
class ReviewService {
    ...
    addReview(review,itemId){
        return axios.post(REVIEW_API_BASE_URL + "addReview/" + itemId, review, { headers: authHeader() });
    }

    getReviewByProductId(productId){
        return axios.get(REVIEW_API_BASE_URL + "reviewByProductId/" + productId,{ headers: authHeader() });
    }
